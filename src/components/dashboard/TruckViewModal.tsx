...
const TruckViewModal: React.FC<TruckViewModalProps> = ({ isOpen, onClose, truck, isDark }) => {
  ...
  const images = [
    truck.main_image,
    truck.image1, truck.image2, truck.image3,
    truck.image4, truck.image5, truck.image6,
    truck.image7, truck.image8, truck.image9
  ].filter(Boolean);
            ...
            <div className="absolute bottom-16 left-1/2 -translate-x-1/2 flex gap-2">
              {images.map((img, idx) => (
                <button
                  key={idx}
                  onClick={() => setCurrentImageIndex(idx)}
                  className={`w-16 h-12 rounded-lg overflow-hidden border-2 transition-all ${idx === currentImageIndex
